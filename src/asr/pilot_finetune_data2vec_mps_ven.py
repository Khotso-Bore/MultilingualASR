"""Pilot data2vec-audio fine-tune for Tshivenda on Apple Silicon (MPS).

Sixth ASR model family attempt. Three CTC fine-tunes have now collapsed
into predicting a single dominant token on every frame - AfriHuBERT (masked
cluster prediction), MMS (Wav2Vec2-style contrastive quantization), w2v-BERT
(hybrid contrastive + masked prediction) - see notes/pilot-ven-results.md.
All three of those objectives discretize their pretraining target in some
way (cluster IDs, quantized codebook entries, or both). data2vec's objective
is different in kind, not just in data: it regresses onto continuous,
contextualized teacher representations (an exponential-moving-average
teacher network's own hidden states), no discretization anywhere. This
pilot is a direct test of whether "discretized pretraining target" is the
common thread behind the collapses, using the last genuinely different
self-supervised paradigm available in transformers for speech.

Honest caveat before running: `facebook/data2vec-audio-large` is pretrained
on Librispeech only (960h, English). No multilingual pretraining at all -
the weakest cross-lingual transfer prior of any checkpoint tried so far
(XLS-R/MMS/w2v-BERT all had broad multilingual pretraining; Whisper's
multilingual data included some African languages even without a Tshivenda
token). If this collapses too, that's still informative (rules out
"discretization" as the sole explanation) but a same-language-family
multilingual data2vec checkpoint was not available to test instead - see
the module docstring history / notes for what was checked.

Reuses the existing tokenizers/ven/ CTC tokenizer and a standard
Wav2Vec2FeatureExtractor (raw waveform input, like XLS-R/MMS - unlike
w2v-BERT's log-mel features) - this script is close to a line-for-line
copy of pilot_finetune_wav2vec2_mps_ven.py with the model class and
checkpoint swapped.
...
Usage:
    PYTORCH_ENABLE_MPS_FALLBACK=1 python src/asr/pilot_finetune_data2vec_mps_ven.py
...
OUTPUT_DIR = REPO_ROOT / "results" / "data2vec-ven-pilot"
BASE_CHECKPOINT = "facebook/data2vec-audio-large"


def main(args):
    device = "mps" if torch.backends.mps.is_available() else "cpu"
    print(f"data2vec-audio pilot fine-tune | device: {device} | train clips: {args.train_clips} | "
    ...
    if args.resume_from:
        ...
        model = Data2VecAudioForCTC.from_pretrained(args.resume_from)
    ...
    print("(w2v-BERT pilot v1: collapse to 'n', WER 0.953/CER 0.933)")
